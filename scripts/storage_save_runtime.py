    ...
    for c in range(class_size):
        ...
        for count in (sf, inf):
            ...
            for _ in range(count):
                diff, p, _ = _uleb(dex, p); _, p, _ = _uleb(dex, p); idx += diff
        for count in (direct, virtual):
            idx = 0
            for _ in range(count):
                diff, p, _ = _uleb(dex, p); _, p, _ = _uleb(dex, p)
                code_off, p, _ = _uleb(dex, p); idx += diff
                if code_off:
                    code[idx] = code_off
...
def _make_void_noop(data: bytearray, code_off: int) -> None:
    """Turn a void method into one valid return-void code item.

    Replacing only the first code unit is unsafe when the original instruction
    was multi-unit: its operand could become a fake opcode.  Shrinking the
    logical code item makes all old bytes an ignored gap and removes stale try
    and debug metadata from the method.
    """
    struct.pack_into('<H', data, code_off + 6, 0)   # tries_size
    struct.pack_into('<I', data, code_off + 8, 0)   # debug_info_off
    struct.pack_into('<I', data, code_off + 12, 1)  # insns_size
    struct.pack_into('<H', data, code_off + 16, 0x000E)  # return-void


def _is_void_noop(data: bytes, code_off: int) -> bool:
    return (
        _u16(data, code_off + 6) == 0 and
        _u32(data, code_off + 8) == 0 and
        _u32(data, code_off + 12) == 1 and
        _u16(data, code_off + 16) == 0x000E
    )
...
def verify_owned_local_save(dex: bytes) -> dict:
    ...
    for cls, name in CLOUD_UI_VOID_METHODS:
        ...
        if not _is_void_noop(dex, code[mid]):
...
def patch_owned_local_save(dex: bytes) -> bytes:
    ...
    _, protos, methods, code = _index(dex)
    ...
    _set_code_unit(out, main_code, 0x59, 0x0412)  # const/4 v4,#0 -> local path
